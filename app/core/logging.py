...
class DetailedFileFormatter(logging.Formatter):
    ...
    def format(self, record):
        ...
        # Add detailed information only if available
        extra_details = []
        ...
        if hasattr(record, 'query_params') and record.query_params:
            extra_details.append(f"Query Params: {self._format_dict(record.query_params)}")
            
        if hasattr(record, 'request_body') and record.request_body:
            extra_details.append(f"Request Body: {self._format_dict(record.request_body)}")
            
        # Response details
        if hasattr(record, 'response_body') and record.response_body:
            extra_details.append(f"Response Body: {self._format_dict(record.response_body)}")
        ...
        # Add the details to the log message if we have any
        if extra_details:
            log_message += "\n" + "\n".join(extra_details)
...
def setup_logging(log_level: str = "INFO") -> None:
    ...
    # File handler - detailed format for requests/responses with UTF-8 encoding
    ...
    # Error file handler - detailed format with UTF-8 encoding
    ...
    if settings.ENABLE_TELEGRAM_LOGGING and settings.TELEGRAM_BOT_TOKEN and settings.TELEGRAM_CHAT_ID:
