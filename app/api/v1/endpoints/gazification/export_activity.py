...
async def export_activity_to_excel(
    ...
    try:
        ...
        for activity in activities:
            # Форматируем дату с учетом часового пояса (+7 часов)
        ...
        with pd.ExcelWriter(file_path, engine='xlsxwriter') as writer:
            ...
            # Формат для числовых данных (количество)
            ...
            for row_num in range(1, len(df) + 1):
                for col_num in range(len(df.columns)):
                    ...
                    # Для столбца "Дата входа" применяем формат даты
                    if column_name == 'Дата входа' and cell_value:
                        try:
                            ...
                            worksheet.write(row_num, col_num, cell_value, cell_format)
                    # Для столбца "Количество внесений" применяем числовой формат
            ...
            for col_num, column in enumerate(df.columns):
                ...
                worksheet.set_column(col_num, col_num, min(column_width, 30))  # Ограничиваем максимальную ширину
            
            log_db_operation("export", "Activity Excel", {
